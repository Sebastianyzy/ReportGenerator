...
def generate_monthly_aging_report(filename, workbook, sheet, customercsv, customerjson, aged_receivables_summary, line_num, font_style, font_size, row_height):
    customer_data = construct_customer_dictionary(customercsv, customerjson)
    df = pd.DataFrame(pd.read_excel(aged_receivables_summary))
    # starting line
    output_file_row_num = line_num
